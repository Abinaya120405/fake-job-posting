...
@st.cache_resource(show_spinner="🏷️ Labelling dataset jobs with PAC fraud-detection…")
def build_ml_engine():
    """
    Two responsibilities:
      1. Run detect_fake_job() on every RAW_JOBS row → ml_label, ml_confidence columns
         (uses the pre-trained PAC model from pac_model.pkl)
      2. Build a lightweight TF-IDF cosine-similarity index for seeker job matching
         (separate from the fraud-detection model — purely for recommendation ranking)
    ...
    # ── 1. PAC fraud detection on each dataset row ────────────────────────
    vectorizer, scaler, model, meta_cols = load_ml_artifacts()
    ml_labels, ml_confs = [], []
    ...
    for _, row in df.iterrows():
        text_dict = {
            ...
            "requirements":    "",
            "salary_range":    str(row["salary"]),
            "location":        str(row["location"]),
            "industry":        str(row["industry"]),
            "company_profile": str(row["company"]),
        }
        if model is not None:
            lbl, conf = detect_fake_job(text_dict)
        else:
            lbl, conf = "pending", 0.0
    ...
    # ── 2. Separate cosine-similarity index for seeker job MATCHING ───────
    #    (mirrors the matching tfidf from original build_ml_engine)
    df["corpus"] = (df["title"] + " " +
...
def recommend_jobs(seeker_text: str, df, tfidf, tfidf_matrix, top_n: int = 5):
    """Top-N GENUINE jobs ranked by cosine similarity to seeker's profile text."""
    return tfidf_search(seeker_text, df, tfidf, tfidf_matrix, only_genuine=True).head(top_n)
...
def page_home():
    ...
    genuine_count = int((df["ml_label"] == "genuine").sum()) if "ml_label" in df.columns else len(RAW_JOBS)
...
def page_dashboard_seeker():
    ...
    if section == "📊 Overview":
        genuine_ct = int((df["ml_label"]=="genuine").sum()) if "ml_label" in df.columns else len(RAW_JOBS)
        st.markdown(f"""
        <div class="stat-row">
